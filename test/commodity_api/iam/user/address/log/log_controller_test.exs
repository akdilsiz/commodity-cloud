...
## 
defmodule Commodity.Api.Iam.User.Address.LogControllerTest do
	...
	test "options/2", %{conn: conn} do
		conn = options conn, iam_user_address_log_path(conn, :index, 1, 1)

		assert conn.status == 204
	end

	test "list all user address logs with given identifiers", %{conn: conn} do
		user = Factory.insert(:user)
		address = Factory.insert(:user_address, user: user)

		Factory.insert_list(50, :user_address_log, user: user, address: address)

		conn = get conn, iam_user_address_log_path(conn, :index, user.id, address.id)

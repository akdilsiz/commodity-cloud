...
## 
defmodule Commodity.Api.Iam.User.Email.LogControllerTest do
	...
	test "options/2", %{conn: conn} do
		conn = options conn, iam_user_email_log_path(conn, :index, 1, 1)

		assert conn.status == 204
	end

	test "list all user email logs with given identifiers", %{conn: conn} do
		user = Factory.insert(:user)
		email = Factory.insert(:user_email, user: user)

		Factory.insert_list(50, :user_email_log, user: user, email: email)

		conn = get conn, iam_user_email_log_path(conn, :index, user.id, email.id)
